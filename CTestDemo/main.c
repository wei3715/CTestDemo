...
int main(int argc, const char * argv[]) {
    ...
    int swapA = 5,swapB = 6;
    swap1(swapA, swapB);
    printf("函数外部交换后swapA==%d，swapB==%d\n",swapA,swapB);
    
    //这个只是对指针变量的副本进行了交换，仍然没有改变原来内存存储的值
    void swap2 (int *a, int *b);
    int *pointerA = &swapA,*pointerB = &swapB;
    printf("函数外交换前指针变量pointerA==%p,pointerB==%p\n",pointerA,pointerB);
    swap2(pointerA, pointerB);
    printf("函数外交换后变量值swapA==%d,swapB==%d\n",swapA,swapB);
...
void swap1 (int a, int b){
    ...
    a = b;
    b = temp;
    printf("函数内交换后a==%d,b==%d\n",a,b);
}

//交换的只是原参数指针变量另行拷贝的副本内存内容
void swap2 (int *a, int *b){
    printf("函数内交换前指针变量a==%p,b==%p\n",a,b);
    int *temp = a;
    a = b;
    b = temp;
    printf("函数内交换后指针变量a==%p,b==%p\n",a,b);
